...
class GoogleDriveWrapper:
    ...
    def __init__(self):
        self.auth_interval = 1200
        self.auth_timestamp = 0
        self.authenticate()
        self._cache_data = dict()

    def authenticate(self):
        if time() - self.auth_timestamp > self.auth_interval:
    ...
    def _fetch_dir(self, path):
        if self._cache_data.get(path, None) is None:
            ...
            if path == '/':
                ...
                self._fetch_dir(parent)

                parent_data = self._cache_data.get(parent, None)
                if parent_data is None:
                    raise ValueError('Parent data for path "%s" not found', parent)
                id = None
                for file in parent_data:
                    if file['title'] == name:
                        id = file['id']
                        break
                if id is None:
                    raise ValueError('Could not get "%s" from parent "%s"' % (name, parent))
            data = self.drive.ListFile({'q': "trashed=false and '%s' in parents" % id}).GetList()
            self._cache_data[path] = data
    ...
    def _get_path_id(self, path):
        ...
        if path == '/':
            ...
            parent_data = self._cache_data.get(parent, None)
            if parent_data is None:
                raise ValueError('Could not get the parent directory for path "%s"' % parent)
            parent_id = None
            for file in parent_data:
                if file['title'] == name:
                    parent_id = file['id']
            ...
            if parent_id is None:
                raise ValueError('Could not get the directory info for path "%s"' % parent)
        return parent_id

    def _clear_cache(self, path):
        parent, _ = self._get_parent_and_name(path)
        self._cache_data[path] = None
        if path != '/':
            self._clear_cache(parent)

    def list_dir(self, path):
        self._fetch_dir(path)
        return self._cache_data.get(path, None)
    ...
    def create_dir(self, path):
        ...
        dir_info = self.drive.CreateFile(meta)
        dir_info.Upload()
        self._clear_cache(parent)
    ...
    def path_exists(self, path):
        ...
        try:
            ...
            return False
        parent_dir = self._cache_data.get(parent, None)
    ...
    def upload_file(self, path_local, path_remote):
        parent, name = self._get_parent_and_name(path_remote)
        parent_id = self._get_path_id(parent)
        meta = {'title': name, 'parents': [{'id': parent_id}]}
        uploaded = self.drive.CreateFile(meta)
        uploaded.SetContentFile(path_local)
        uploaded.Upload()
        self._cache_data[parent] = None

    def download_file(self, path_local, path_remote):
        file_id = self._get_path_id(path_remote)
        meta = {'id': file_id}
        downloaded = self.drive.CreateFile(meta)
        downloaded.GetContentFile(path_local)

    def delete_path(self, path, trash=True):
        parent, _ = self._get_parent_and_name(path)
        file_id = self._get_path_id(path)
        meta = {'id': file_id}
        deleted = self.drive.CreateFile(meta)
        if trash:
            deleted.Trash()
        else:
            deleted.Delete()
...
class DCGAN:
    ...
    def __init__(self, training_set_path, batch_size=128, image_width=100, image_height=100, channel_count=3,
        ...
        # variables for syncing to google drive
        self._sync_thread = None
        self._sync_required = None
        self._sync_finished = None
    ...
    def train_model(self, sync_drive_wrapper=None):
        ...
        # async uploader for google drive
        self._sync_required = threading.Event()
        self._sync_finished = threading.Event()
        if sync_drive_wrapper is not None:
            self._sync_thread = threading.Thread(target=self._sync_model, args=(sync_drive_wrapper,))
        ...
        while True:
            ...
            for _ in tqdm(range(steps)):
                ...
                if self._step % self.save_model_per_step == 0:
                    self._save_model(self.weight_dir, sync_drive_wrapper)
        ...
        self._save_model(self.weight_dir, sync_drive_wrapper)
        self._sync_finished.wait()

    def _cleanup_model_dir(self, max_keep_items=5):
        print('Cleaning up model directory...')
        files = os.listdir(self.weight_dir)
        training_steps = dict()
        for file in files:
            ...
            if step is None:
                continue
            step = step[1]
            training_steps[int(step)] = True
        training_steps = list(training_steps.keys())
        training_steps.sort()
        training_steps = training_steps[-max_keep_items:]
        for file in files:
            step = re.search(re.compile('.*?(\\d+)\\..*'), file)
            if step is None:
                continue
            ...
            if int(step) not in training_steps:
                path = os.path.join(self.weight_dir, file)
                print('removed %s' % path)
                os.remove(path)

    def _sync_model(self, sync_api):
        while True:
            try:
                self._sync_required.wait()
                self._sync_required.clear()
                print('Syncing model data to google drive...')
                self._cleanup_model_dir()
                if not sync_api.path_exists('/tf_colab'):
                    sync_api.create_dir('/tf_colab')

                def _sync_dir(local_dir, remote_dir, overwrite=False, remove_remote=False):
                    local_files = os.listdir(local_dir)
                    remote_files = sync_api.list_dir(remote_dir)
                    remote_files = [x['title'] for x in remote_files]
                    for file in local_files:
                        if file not in remote_files:
                            sync_api.upload_file(os.path.join(local_dir, file), remote_dir + '/' + file)
                            print('uploaded file: %s/%s' % (remote_dir, file))
                        elif overwrite:
                            sync_api.delete_path(remote_dir + '/' + file, False)
                            sync_api.upload_file(os.path.join(local_dir, file), remote_dir + '/' + file)
                            print('uploaded file: %s/%s' % (remote_dir, file))
                    ...
                    if remove_remote:
                        for file in remote_files:
                            if file not in local_files:
                                sync_api.delete_path(remote_dir + '/' + file, False)
                                print('deleted file: %s/%s' % (remote_dir, file))

                if not sync_api.path_exists('/tf_colab/' + self.weight_dir):
                    sync_api.create_dir('/tf_colab/' + self.weight_dir)
                    print('created remote directory: /tf_colab/' + self.weight_dir)
                if not sync_api.path_exists('/tf_colab/' + self.log_dir):
                    sync_api.create_dir('/tf_colab/' + self.log_dir)
                    print('created remote directory: /tf_colab/' + self.log_dir)
                if not sync_api.path_exists('/tf_colab/' + self.output_dir):
                    sync_api.create_dir('/tf_colab/' + self.output_dir)
                    print('created remote directory: /tf_colab/' + self.output_dir)

                _sync_dir(self.weight_dir, '/tf_colab/' + self.weight_dir, True, True)
                _sync_dir(self.log_dir, '/tf_colab/' + self.log_dir, True)
                _sync_dir(self.output_dir, '/tf_colab/' + self.output_dir)
...
def main():
    ...
    api = GoogleDriveWrapper()
    if not os.path.exists(training_set_path):
        api.download_file('animeface-dataset.7z', '/animeface-dataset.7z')
        !7z x animeface-dataset.7z
    else:
        print('ignored: training set directory has already existed')

    if not api.path_exists('/tf_colab'):
        api.create_dir('/tf_colab')
    print('retrieving model weights...')
    if api.path_exists('/tf_colab/' + weight_dir):
        if not os.path.exists(weight_dir):
            os.mkdir(weight_dir)
        for file in api.list_dir('/tf_colab/' + weight_dir):
            api.download_file(weight_dir + '/' + file['title'], '/tf_colab/' + weight_dir + '/' + file['title'])
            print('downloaded %s (%s)' % (file['title'], file['id']))
    else:
        print('ignored: remote directory not exists')
