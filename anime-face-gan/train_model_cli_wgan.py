...
class WGAN:
    ...
    def _discriminator_model(self, input_tensor, reuse=False):
        with tf.variable_scope('discriminator') as scope:
            ...
            model = tf.reshape(model, [tf.shape(model)[0], np.prod(model.get_shape().as_list()[1:])],
                               name='layer5/flatten')
            model_logits = dense(model, 1, name='layer5/dense', use_bias=True)
    ...
    def _compile_loss(self):
        ...
        d_loss = tf.reduce_mean(self._d_logits - self._gan_logits)
        epsilon = tf.random_uniform(shape=[tf.shape(self._d_logits)[0], 1, 1, 1], minval=0, maxval=1)
        x_hat = self._input_tensor + epsilon * (self._g - self._input_tensor)
        d_x_hat = self._discriminator_model(x_hat, True)
